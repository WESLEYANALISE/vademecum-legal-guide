...
const HeroCarousel = () => {
  ...
  if (noticias.length === 0) {
    // Show hero image immediately even without news
    return (
      <div className="relative h-52 sm:h-56 overflow-hidden">
        ...
        <div className="relative z-10 h-full flex flex-col justify-end px-4 pb-8 sm:px-6">
          <h2 className="font-display text-lg sm:text-xl font-bold text-white leading-tight drop-shadow-lg">Vade Mecum 2026</h2>
          <p className="text-white/70 text-sm mt-1">Sua legislação completa e anotada</p>
        </div>
        {/* Logo */}
        ...
        {/* Novidades */}
        <button onClick={() => { setBadgeCount(0); navigate('/novidades'); }} className="absolute top-3 right-4 z-20 w-9 h-9 rounded-full bg-black/30 backdrop-blur-sm flex items-center justify-center border border-white/10 hover:bg-black/50 transition-colors">
          <Bell className="w-[18px] h-[18px] text-white" />
          {badgeCount > 0 && <span className="absolute -top-1 -right-1 min-w-[16px] h-4 rounded-full bg-primary text-primary-foreground text-[9px] font-bold flex items-center justify-center px-1">{badgeCount > 99 ? '99+' : badgeCount}</span>}
        </button>
      </div>
    );
  }
